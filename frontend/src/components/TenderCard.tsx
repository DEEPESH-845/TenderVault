...
export default function TenderCard({ tender, userInfo }: TenderCardProps) {
  ...
  return (
    <Link to={`/tenders/${tender.tenderId}`} className="block">
      <div className="card group cursor-pointer hover:border-vault-200 hover:shadow-vault-100/50">
        {/* Header */}
        ...
        {/* Title */}
        <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-vault-700 transition-colors line-clamp-2">
          {tender.title}
        </h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-50">
          ...
          {userInfo?.role === 'tv-bidder' && tender.status === 'OPEN' && isLocked && (
            <span className="text-xs font-semibold text-vault-600 bg-vault-50 px-2.5 py-1 rounded-full">
              Submit Bid →
            </span>
          )}
