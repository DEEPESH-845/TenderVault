  ...
  return (
    <div ref={revealRef}>
      <Link to={`/tenders/${tender.tenderId}`} className="block">
        <div className="card tilt-card group cursor-pointer hover:border-vault-200 hover:shadow-vault-100/50 dark:hover:border-vault-500/30 dark:hover:shadow-vault-900/30">
          {/* Header */}
          <div className="flex items-start justify-between mb-3">
            ...
            {isLocked && tender.status === 'OPEN' && (
              <div className="flex items-center gap-1.5 text-amber-600 animate-lock-pulse">
                ...
                <span className="text-xs font-bold font-mono tabular-nums">{countdown}</span>
              </div>
            )}
          </div>
